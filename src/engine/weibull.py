...
if __name__ == '__main__':
    data = spark.read.csv('../data/weibull_data.csv', header=True, inferSchema=True)
    sim = Weibull(data=data).fit(states=[5,15], values=[0.95, 0.05], model_name='default')
    out = sim.run(n_steps=5)
